                    ...
                    @forelse ($workflows as $index => $workflow)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/40">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center">
                                    <span class="inline-flex items-center gap-2 rounded-full bg-blue-600 px-3 py-2 text-sm font-semibold text-white shadow-md shadow-blue-200/70 dark:shadow-none">
                                        @php
                                            $hasStand = str_contains($workflow->slug, 'stand');
                                            $hasOnline = str_contains($workflow->slug, 'online');
                                        @endphp
                                        @if ($hasStand && $hasOnline)
                            ...
                            <td class="px-5 py-4 text-right whitespace-nowrap">
                                @foreach (['stand', 'online'] as $channel)
                                    @if (str_contains($workflow->slug, $channel))
                                        <a href="{{ route('products.workflow.index', $channel) }}" class="inline-flex rounded-lg p-2 text-blue-600 hover:bg-blue-50 dark:text-blue-400 dark:hover:bg-gray-700" title="View {{ ucfirst($channel) }} products">
