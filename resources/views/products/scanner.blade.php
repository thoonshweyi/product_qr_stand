...
@section('content')
<div class="min-h-screen bg-gradient-to-b from-slate-100 via-white to-blue-50 pb-16 pt-28 text-slate-900 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 dark:text-white">
    <main class="mx-auto w-full max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="overflow-hidden rounded-3xl bg-white shadow-2xl ring-1 ring-slate-200/70 dark:bg-slate-800 dark:ring-slate-700">
            <div class="grid lg:grid-cols-[1.15fr_.85fr]">
                <section class="relative bg-slate-950 p-4 sm:p-7" aria-labelledby="scanner-title">
                    <div class="mb-5 flex items-start justify-between gap-4 text-white">
                        <div>
                            <p class="text-xs font-bold uppercase tracking-[0.2em] text-blue-300">Product finder</p>
                            <h1 id="scanner-title" class="mt-1 text-2xl font-bold sm:text-3xl">Scan QR Code</h1>
                        </div>
                        <div id="camera-indicator" class="flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs font-semibold text-slate-300">
                            <span id="camera-dot" class="h-2 w-2 rounded-full bg-slate-500"></span>
                            <span id="camera-label">Camera off</span>
                        </div>
                    </div>

                    <div class="relative min-h-[320px] overflow-hidden rounded-2xl border border-white/10 bg-slate-900 sm:min-h-[400px]">
                        <div id="qr-reader" class="h-full w-full"></div>
                        <div id="camera-placeholder" class="absolute inset-0 flex flex-col items-center justify-center px-8 text-center">
                            <div class="flex h-20 w-20 items-center justify-center rounded-3xl bg-blue-500/15 text-blue-300 ring-1 ring-blue-400/30">
                            ...
                            <p class="mt-5 font-semibold text-white">Camera is ready when you are</p>
                            <p class="mt-2 max-w-sm text-sm leading-6 text-slate-400">Allow camera access, then place the product QR code inside the scanning frame.</p>
                        </div>
                    </div>

                    <div class="mt-5 flex flex-wrap justify-center gap-3">
                        <button id="start-scanner" type="button" class="inline-flex min-w-[160px] items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-blue-950/30 transition hover:bg-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-400/30">
                        ...
                        <button id="stop-scanner" type="button" disabled class="inline-flex min-w-[130px] items-center justify-center gap-2 rounded-xl border border-white/20 bg-white/10 px-5 py-3 text-sm font-bold text-white transition hover:bg-white/15 disabled:cursor-not-allowed disabled:opacity-40">
                ...
                <aside class="flex flex-col p-6 sm:p-8 lg:p-10">
                    <div>
